...
# # === Simple FAISS RAG search ===
# def search_faiss_idx(query_embedding: list, idx, texts: list, top_k: int = 8) -> list:
#     emb = np.array(query_embedding, dtype="float32").reshape(1, -1)
#     distances, indices = idx.search(emb, top_k)
...
#     context_texts = search_faiss_idx(embedding, index_cache, texts_cache)
#     context = "\n".join(context_texts)
#     return gemini_generate_response(context, query)
#
# def response_with_judgement(query: str) -> str:
#     try:
#         answer_1 = llama_response(query)
#         answer_2 = llama_response(query)
#
#         if not answer_1.strip() or not answer_2.strip():
#             return "⚠️ 未成功產生回覆，請稍後再試。"
#
...
#             return answer_2
#         else:
#             return answer_1  # fallback
#     except Exception as e:
#         print(f"[Judgement Error] {e}")
#         return "⚠️ 系統處理失敗，請稍後再試。"
